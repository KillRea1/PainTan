...
class Canvas(QLabel):
    ...
    # Общие события (общих кисти инструменты)
    ...
    def fill_mousePressEvent(self, e):
        ...
        x, y = e.x(), e.y()
        # Найдем 3-байтовое значение в нашем текущем местоположении.
        i = (x + (y * w)) * 4
        target_color = s[i:i+3]

	#yfcnhjqrb jnj,hf;tybz
    ...
    def dropper_mousePressEvent(self, e):
        ...
        if e.button() == Qt.LeftButton:
            ...
            self.primary_color_updated.emit(hex)  # Обновление UI.

        elif e.button() == Qt.RightButton:
            self.set_secondary_color(hex)
            self.secondary_color_updated.emit(hex)  # Обновление UI.

    # Общие события формы: прямоугольник, круг
    ...
    def generic_poly_mouseDoubleClickEvent(self, e):
        ...
        # Делаем так чтобы щетка не убирала выделение
...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # запись для изменений цвета (пипетка).
    ...
    def open_file(self):
        ...
        if path:
            ...
            if iw/cw < ih/ch:  # Высота относительно меньше ширины.
            ...
            elif iw/cw > ih/ch:  # Высота относительно больше, чем ширина
